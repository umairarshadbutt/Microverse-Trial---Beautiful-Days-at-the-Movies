...
process.stdin.on('end', _ => {    inputString = inputString.replace(/\s*$/, '')        .split('\n')        .map(str => str.replace(/\s*$/, ''));
    main();});
function readLine() {    return inputString[currentLine++];}
// Complete the beautifulDays function below.function beautifulDays(i, j, k) {    var count=0;    for (let start= i; start<=j ; start++){        let reverse= parseInt(start.toString().split('').reverse().join(''));        let temp= Math.abs(start - reverse);        if (temp % k === 0)        {            count++;        }    }
    return count;
}
